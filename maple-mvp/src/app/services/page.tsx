...
import ServiceFilters, { ServiceFiltersState } from "@/components/ServiceFilters";
...
export default function ServicesPage() {
  ...
  const [filters, setFilters] = useState<ServiceFiltersState>({ q: "", type: "all" });
  return (
    <section className="max-w-6xl mx-auto px-4 py-10 space-y-6">
      <h1 className="text-3xl font-bold mb-2">Services</h1>
      <p className="text-gray-700 dark:text-gray-300">Curated Service Canada and provincial service centres. Filter by type or search. For full directories, see the official sites below.</p>
      <ServiceFilters value={filters} onChange={setFilters} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <Map province={province} filters={filters} />
        </div>
        <div>
          <MarkerList province={province} filters={filters} />
        </div>
      </div>
