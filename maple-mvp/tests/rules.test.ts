...
const base: Profile = { audience: "newcomer", province: "on", status: "pr", goals: ["healthcare", "ids"] };
...
describe("rules engine", () => {
  ...
  it("localizes titles to French", () => {
    ...
    expect((sin?.title as string) || "").toMatch(/numéro d’assurance sociale|NAS/i);
  });

  it("sorts by goal order", () => {
    const p: Profile = { audience: "newcomer", province: "on", status: "pr", goals: ["healthcare", "ids"] };
    const steps = generateChecklist(p, "en");
    expect(steps[0].goal).toBe("healthcare");
  });

  it("filters by selected goals", () => {
    const p: Profile = { audience: "newcomer", province: "on", status: "pr", goals: ["ids"] };
    const steps = generateChecklist(p, "en");
    expect(steps.every((s) => s.goal === "ids")).toBe(true);
  });

  it("filters by province", () => {
    const p: Profile = { audience: "newcomer", province: "bc", status: "pr", goals: ["healthcare", "ids"] };
    const steps = generateChecklist(p, "en");
  ...
  it("includes Quebec steps for qc", () => {
    const p: Profile = { audience: "newcomer", province: "qc", status: "pr", goals: ["healthcare", "ids"] };
    const steps = generateChecklist(p, "en");
